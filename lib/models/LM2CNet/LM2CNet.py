...
class Mono3DVG(nn.Module):
    ...
    def forward(self, images, calibs,  img_sizes, text, im_name, instanceID, ann_id, text_index):
        """ The forward expects a NestedTensor, which consists of:
               - samples.tensor: batched images, of shape [batch_size x 3 x H x W]
               - samples.mask: a binary mask of shape [batch_size x H x W], containing 1 on padded pixels
        """
        text_index = text_index.cpu()
        text_index = text_index.numpy().astype(dtype=int).tolist()
        text = [text[x] for x in text_index]
        ...
        if isinstance(text[0], str):
            ...
            position_ids = tokenized['input_ids']
            seq_length = position_ids.size(1)

            text_position = torch.arange(seq_length).expand(len(text), seq_length).to(position_ids.device)

            text_position_emb = self.text_encoder.embeddings.position_embeddings(text_position).transpose(0, 1)
            ...
            text_position_emb_resized = self.resizer(text_position_emb)
        else:
        ...
        out['depth_weight'] = weighted_depth
