...
from .text_fuse import DynamicWeightedFusion
...
class TextGuidedAdapter(nn.Module):
    ...
    def forward(self, img_feat_src, masks, img_pos_embeds,
        ...
        k = self.with_pos_embed(word_feat, word_pos.transpose(0, 1))
        ...
        depthfeat_adapt = self.depth2textcross_attn(
            ...
            key=self.with_pos_embed(word_feat, word_pos.transpose(0, 1)).transpose(0, 1),
...
class LM2CNetTransformer(nn.Module):
    def __init__(
            self,
            d_model=256,
            nhead=8,
        ...
        self.sk = DynamicWeightedFusion(input_dim=256)

        decoder_layer = LM2CNetDecoderLayer(
            d_model, dim_feedforward, dropout, activation, num_feature_levels, nhead, dec_n_points)
        self.decoder = LM2CNetDecoder(decoder_layer, num_decoder_layers, return_intermediate_dec)
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None, depth_pos_embed=None,
                text_memory=None, text_mask=None, text_pos=None, im_name=None, instanceID=None, ann_id=None):
        ...
        img_feat_orig2adapt, depth_feat_orig2adapt = self.TextGuidedAdapter(memory, mask_flatten, lvl_pos_embed_flatten,
                ...
                level_start_index,
                valid_ratios,
                text_memory, text_mask, depth_pos_embed, mask_depth, im_name, instanceID, ann_id, text_pos)
        ...
        hs, inter_references, inter_references_dim = self.decoder(
            ...
            depth_adapt_k,
            mask_depth,
            text_memory, text_mask, bs, text_pos)
...
class VisualEncoderLayer(nn.Module):
    def __init__(self,
        ...
        ##############################
        self.self_atten_text = nn.MultiheadAttention(d_model, n_heads, dropout=dropout)
        self.text_norm = nn.LayerNorm(d_model)
        self.depth2text = nn.MultiheadAttention(d_model, n_heads, dropout=dropout)
        self.depth2text_norm = nn.LayerNorm(d_model)
    ...
    def text_ffn(self, text):
        text2 = self.linear2(self.dropout2(self.activation(self.linear1(text))))
        text = text + self.dropout3(text2)
        text = self.norm2(text)
        return text

    def forward(self, src, pos, reference_points, spatial_shapes, level_start_index, text_memory, text_mask, padding_mask=None, mask_flatten=None, text_pos=None, depth_pos_embed=None, mask_depth=None):
        # Multi-Scale Deformable Attention
        ...
        q = k = self.with_pos_embed(text_memory, text_pos.transpose(0, 1))
        text = self.self_atten_text(
            query=q,
            key=k,
            value=text_memory
        )[0]
        text_memory = self.text_norm(text_memory + text)

        text2 = self.depth2text(
            query=self.with_pos_embed(text_memory, text_pos.transpose(0, 1)).transpose(0, 1),
            key=depth_pos_embed,
            value=depth_pos_embed,
            key_padding_mask=mask_depth
        )[0].transpose(0, 1)
        text_memory = self.depth2text_norm(text2 + text_memory)
        text_memory = self.text_ffn(text_memory)
        # cross attention

        src3 = self.cross_attn(query=self.with_pos_embed(src, pos).transpose(0, 1),
                               key=self.with_pos_embed(text_memory, text_pos.transpose(0, 1)).transpose(0, 1),
...
class VisualEncoder(nn.Module):
    ...
    def forward(self, src, spatial_shapes, level_start_index, valid_ratios,text_memory, text_mask,
                pos=None, padding_mask=None, text_pos=None, depth_pos_embed=None, mask_depth=None):
        output = src
        text_output = text_memory
        reference_points = self.get_reference_points(spatial_shapes, valid_ratios, device=src.device)
        for _, layer in enumerate(self.layers):
            output, text_output = layer(output, pos, reference_points, spatial_shapes, level_start_index,text_output, text_mask, padding_mask, text_pos=text_pos, depth_pos_embed=depth_pos_embed, mask_depth=mask_depth)

        return output, text_output


class LM2CNetDecoderLayer(nn.Module):
    ...
    def forward(self,
        ...
        #
        # if self.training:
        #     num_noise = num_queries - self.group_num * 1
        #     num_queries = self.group_num * 1
        ...
        tgt_text = self.cross_attn_text(query=self.with_pos_embed(tgt2, query_pos).transpose(0, 1),
                                        key=self.with_pos_embed(text_memory, text_pos.transpose(0, 1)).transpose(0, 1),
                                        value=text_memory.transpose(0, 1),
...
class LM2CNetDecoder(nn.Module):
    ...
    def forward(self, tgt, reference_points, src, src_spatial_shapes, src_level_start_index, src_valid_ratios,
                query_pos=None, src_padding_mask=None, depth_pos_embed=None, depth_adapt_k=None, mask_depth=None,text_memory=None, text_mask=None, bs=None, textpose=None):
        output = tgt
        ...
        for lid, layer in enumerate(self.layers):
            ...
            output = layer(output,
                           ...
                           depth_adapt_k,
                           mask_depth,
                           text_memory, text_mask, bs, textpose)
...
def build_LM2CNet_trans(cfg):
    return LM2CNetTransformer(
